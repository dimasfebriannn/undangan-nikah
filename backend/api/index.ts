...
app.get('/api/inv/:slug/guest', async (c) => {
  ...
  const guestResult = await db
    .select().from(guests)
    .where(and(eq(guests.invitationId, invitationResult[0].id), like(guests.name, `%${name}%`)))
    .limit(1)
...
app.post('/api/inv/:slug/rsvp', async (c) => {
  ...
  try {
    ...
    const existingGuest = await db
      .select().from(guests)
      .where(and(eq(guests.invitationId, invitation.id), eq(guests.name, data.name)))
      .limit(1)
...
// POST /api/inv/:slug/admin/guests
...
// POST /api/inv/:slug/admin/guests/bulk
...
// DELETE /api/inv/:slug/admin/guests/:id
